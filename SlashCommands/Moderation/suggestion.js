...
module.exports = {
    ...
    async execute(client, interaction, args){
        ...
        if (SubCommand === "channel") {
            ...
            const CodeSchema = require('../../models/Suggestion/suggestion')
                ...
                if (data) {
                    ...
                    channell.send({
                        ...
                        const replyembed = new MessageEmbed()
                            .setColor('RED')
                            .setDescription(`<:Verified_Developer_Badge_SkyBlue:899282294685188139> Your [Suggestion](https://discord.com/channels/${interaction.guild.id}/${data.Channel}/${m.id}) has been sent!`)
                        interaction.followUp({
                            embeds: [replyembed]
                        })
                        m.react('<:8558upvote:922875612551589920>')
                        m.react('<:9651downvote:922875611897278525>')
                        new CodeSchema({
                            message: m.id,
                            token: pass,
            ...
            const Schema = require('../../models/Suggestion/suggestion')
            Schema.findOne({
                token: stoken,
            }, async (err, data) => {
                if (!data) return interaction.followUp(`${process.env.FAILURE_EMOJI} Invalid Token`)
                const message = data.message
                const user = client.users.cache.get(data.user)
                const guild = data.guild
                const suggestion = data.suggestion

                if (interaction.guild.id !== guild) return interaction.followUp(`${process.env.FAILURE_EMOJI} Invalid Token`)
                ...
                if(!gchannel) return interaction.followUp(`${process.env.FAILURE_EMOJI} Couldnt find the Suggestion Channel.. Please set a new One`)
                if (channel) {
                    const embed = new MessageEmbed()
