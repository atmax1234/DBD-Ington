const warnModel = require("../../models/warn-system")
const { 
  Client, 
  CommandInteraction, 
  MessageEmbed, 
        ...
        collector.on('collect', async (i) => { //collecter
          if(i.customId === 'YES') {
            ...
            await new warnModel({
              ...
              await member.send(`You have been warned in ${interaction.guild.name} for ${reason}`).catch(() =>{
                interaction.followUp({content: '❗User has DMs closed or has no mutual servers with the bot.', ephemeral: true})
              })
 ...
 const embed1 = new MessageEmbed()
        ...
        collector.on('collect', async (i) => {
          if(i.customId === 'YES') {
            ...
            data.delete();
             const user = interaction.guild.members.cache.get(data.userId);
             await user.send(`Your 1 warning has been removed`).catch(() =>{
              interaction.followUp({content: '❗User has DMs closed or has no mutual servers with the bot.', ephemeral: true})
            })
             collector.stop('success')
